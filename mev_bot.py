...
# ✅ Correct way to import PoA Middleware
...
def predict_trade(transaction_data):
    try:
        predicted = model.predict([transaction_data])[0]
        value = transaction_data[0]  # ETH Value
        gas_price = transaction_data[1]
        gas_limit = 210000
        gas_fee_eth = (gas_price * gas_limit) / 10**18
        min_profit = value * 0.002  # Ensure at least 0.2% profit

        # ✅ Only execute if AI predicts profitability and profit > gas fee
        if predicted == 1 and min_profit > gas_fee_eth:
            return True
        return False
    except Exception as e:
...
def execute_trade(chain, transaction):
    ...
    try:
        ...
        if min_profit > gas_fee_eth:
            ...
            # ✅ Properly format the transaction dictionary
            tx = {
                'to': wallet_address,
                'value': int(value),
                'gas': gas_limit,
